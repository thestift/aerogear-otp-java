...
public class Totp {
  ...
  private Hash hashAlgorithm = Hash.SHA1;
  ...
  public Totp(String secret, Clock clock, Hash hashAlgorithm) {
    this(secret, clock);
  ...
  public String uri(String name) {
    try {
      if (this.hashAlgorithm == Hash.SHA1) {
  ...
  public String uri(String appName, String email, String issuer) {
    try {
      return URLEncoder.encode(String.format(
          "otpauth://totp/%s:%s?secret=%s&issuer=%s&algorithm=%s",
          appName, email, secret, issuer, hashAlgorithm.name()),
          "UTF-8");
    } catch (UnsupportedEncodingException e) {
      throw new IllegalArgumentException(e.getMessage(), e);
    }
  ...
  public String uriWithIntervall(String appName, String email, String issuer) {
    try {
      return URLEncoder.encode(String.format(
          "otpauth://totp/%s:%s?secret=%s&issuer=%s&algorithm=%s&period=%s",
          appName, email, secret, issuer, hashAlgorithm.name(), clock.getInterval()),
          "UTF-8");
    } catch (UnsupportedEncodingException e) {
      throw new IllegalArgumentException(e.getMessage(), e);
    }
   ...
   * Author: sweis@google.com (Steve Weis)
   */
  public boolean verify(String otp, int timedWindow) {

    long code = Long.parseLong(otp);
    long currentInterval = clock.getCurrentInterval();

    for (int i = timedWindow * -1; i <= timedWindow; i++) {
      long tempInterval = currentInterval - i;
      int candidate = generate(this.secret, tempInterval);
      if (candidate == code) {
        return true;
      }
    }
  ...
  private int hash(String secret, long interval) {
    ...
    try {
      ...
      hash = new Hmac(hashAlgorithm, Base32.decode(secret), interval).digest();
